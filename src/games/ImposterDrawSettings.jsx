...
import { Check, Shuffle, Brain, ShieldAlert, AlertTriangle } from 'lucide-react';
import { IMPOSTER_DRAW_DATA } from '../data/imposterDrawData';

export default function ImposterDrawSettings({ players, onStartGame }) {
  // --- NEW: MULTI-SELECT CATEGORY STATE ---
  const availableCategories = Object.keys(IMPOSTER_DRAW_DATA);
  ...
  const handleStart = () => {
    onStartGame({
      ...
      gameDuration: 0, // No timer used in drawing mode
      useHints,
      useAltWord,
      useAssistant
    });
    ...
    <div className="w-full h-[100dvh] relative bg-[#050505] overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 h-28 px-6 pt-8 pb-4 flex items-end justify-between bg-gradient-to-b from-black via-black/90 to-transparent z-20 pointer-events-none">
        <div className="pointer-events-auto">
          <h2 className="text-3xl font-display font-black text-white leading-none">DRAWING</h2>
          <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mt-2">Configure Rules</p>
        </div>
        <div className="pointer-events-auto text-xs text-black font-bold uppercase tracking-widest bg-pink-500 px-3 py-1 rounded-full shadow-[0_0_15px_rgba(236,72,153,0.4)]">
      ...
      <div className="absolute inset-0 overflow-y-auto no-scrollbar pt-32 pb-32 px-6 space-y-8">
        ...
        <section>
          <div className="flex justify-between items-end mb-3 pl-1">
              <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">Select Drawing Categories</p>
          </div>
              ...
              return (
                <button
                  ...
                  className={`px-4 py-3 rounded-xl text-xs font-bold transition-all border ${
                    isSelected 
                      ? 'bg-pink-500 text-black border-pink-500 shadow-[0_0_15px_rgba(236,72,153,0.3)] scale-105' 
        ...
        <section className="w-full">
            ...
            <div className="bg-white/5 p-4 rounded-2xl border border-white/10 w-full">
                <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-2">Fake Artists (Imposters)</p>
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button 
                        ...
                        className={`p-2 rounded-lg transition-colors ${isRandomImposters ? 'bg-pink-500 text-white' : 'bg-white/10 text-zinc-500'}`}
                    >
                        <Shuffle size={16} />
                    </button>
                </div>
                {!isRandomImposters && (
                    <input 
                        ...
                        className="w-full accent-pink-500 h-1 bg-black rounded-lg appearance-none cursor-pointer"
                    />
                )}
            </div>
        </section>
        ...
        <section className="space-y-3">
          <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-1 pl-1">Extra Modifiers</p>
          ...
          <ToggleOption 
            ...
            icon={<Brain size={18} />} 
            title="Enable Hints" 
            desc="Fake Artist gets a vague clue." 
            color="blue"
          />
          <ToggleOption 
            active={useAltWord} 
            ...
            title="Confused Fake Artist" 
            desc="Sees a similar wrong word to draw." 
            color="red"
          />
          <ToggleOption 
            active={useAssistant} 
            ...
            desc="Knows the word. Helps the Fake Artist." 
            color="green"
          />
        </section>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 pt-8 bg-gradient-to-t from-black via-black to-transparent z-50">
        <button 
            onClick={handleStart} 
            className="w-full py-4 bg-gradient-to-r from-pink-500 to-purple-600 text-white font-display font-black tracking-widest rounded-2xl shadow-[0_0_25px_rgba(236,72,153,0.3)] active:scale-95 transition-all hover:brightness-110"
        >
            START MATCH
        </button>
      </div>
